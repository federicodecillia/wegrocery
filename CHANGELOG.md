...
- **The last Italian texts on English deployments** — the order-rectification notification (in app and email) and the negative-balance warning after saving an order — now follow the app language.
...
- **"Catalogo in preparazione" empty state in the order form.** When a cycle is open but no products have been loaded yet, members see a friendly 📦 message instead of a blank form that looked broken.
- **Explicit 10 MB cap on admin file uploads** (listino import, product template, distinta), checked before the file is even decoded, plus a decompression-bomb guard on `.ods` parsing. Next.js's default 1 MB body limit already blocked bigger payloads; this makes the guard intentional instead of accidental.
- **Database-level unique indexes** on order lines (one per member × cycle × product) and on per-cycle product identity (the same lower/trimmed name|variant|format|unit key the import dedup uses), so concurrent imports can't slip duplicates past the app-side checks. Migration `drizzle/0008_unique_constraints.sql` — run the pre-flight checks in its header, then `npm run db:push`.
- **Unit tests for the money-adjacent pure functions**: proportional shipping split (cent-exact under rounding drift), changelog parser, supplier-header heuristics, spreadsheet number parsing, plus cycle-access and category-guess coverage. Suite grows to 78 tests.
...
- **Upload a completed order sheet (distinta) as .ods or .csv too.** Admin → Ciclo → 📤 Carica distinta previously accepted only the original `.xlsx`. It now also accepts the same sheet re-saved as `.ods` (e.g. from LibreOffice) or exported as `.csv`. With `.xlsx` and `.ods` the file's hidden mapping is preserved, so matching stays exact; with `.csv` (which can't carry that mapping) members and products are matched by name, and any column or row that can't be matched is flagged in the preview and skipped — never guessed — so you still review every change before applying.

### Fixed
- **Wrong auto-suggested product emojis.** "Melanzana/Melanzane" showed 🍉 (watermelon) instead of 🍆; "Riso/Risotto" got 🍞 (bread) instead of 🍚; "Peperoni" fell through to 🧂 (salt) instead of 🫑. All three were ordering/overlap bugs in the first-match-wins pattern table, now pinned by regression tests.
- **Saving an order is now atomic.** The old delete-then-insert ran as two independent requests, so an interruption in between (network blip, tab closed mid-submit) could silently leave the member's order empty. Both statements now travel in a single batched transaction; an invalid product also aborts *before* the existing order is touched.
- **Corrupted upload files show a localized error** ("File vuoto o illeggibile") in the import wizard instead of the parsing library's raw English message. The last three hardcoded Italian placeholders in the admin UI (treasury note, supplier variant, stats filter search) now follow the app locale too.
