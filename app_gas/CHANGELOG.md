...
- **Supplier .xlsx now alphabetical across every sheet.** The `Distinta` matrix and `Totali per prodotto` used to follow each product's `sortOrder` field, which could be set arbitrarily and rarely matched the order suppliers expected. All three sheets — `Distinta`, `Riepilogo ordini`, `Totali per prodotto` — are now sorted by product name (with variant as tiebreaker), so opening the file the supplier can scan top-to-bottom alphabetically.
